...
# MODELO: MODEL GRID
###############################################################################
# con seleccion de varibles
###############################################################################

ArchivoDeSalida = "modelos_reglog_svm_rf_datos01.txt"
...
grid_modelos <- grid_modelos %>%
  share_settings(
    ...
    x = datos_train_prep %>% select(-deserto) %>% select(rf_rfe$optVariables), # seleccion variables por rf
